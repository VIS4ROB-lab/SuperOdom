...
class ScopedTimer {
 public:
  explicit ScopedTimer(
      ...
      rclcpp::Logger logger = rclcpp::get_logger("ScopedTimer"))
      : name_(name),
...
inline void transformUpdate(const Eigen::Quaterniond& q_w_curr,
...
inline bool compare_pair_first(
    const std::pair<float, int> a,
    const std::pair<float, int> b)  // sort from big to small
{
 ...
 * @return The PCA
 */
inline Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> ComputePCA(
