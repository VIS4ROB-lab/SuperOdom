...
struct Imu {
 ...
 public:
  Imu()
      : Imu(0.0, Eigen::Vector3d(0.0, 0.0, 0.0),
  ...
  // Function to calculate pitch and roll and construct the combined rotation
  // matrix
  Eigen::Matrix3d calculatePitchRollMatrix(double ax, double ay, double az) {
    ...
    Eigen::Matrix3d R_y;
    R_y << std::cos(theta), 0, std::sin(theta), 0, 1, 0, -std::sin(theta), 0,
        std::cos(theta);

    // Construct the roll rotation matrix
    Eigen::Matrix3d R_x;
    R_x << 1, 0, 0, 0, std::cos(phi), -std::sin(phi), 0, std::sin(phi),
        std::cos(phi);
  ...
  void imuInit(MapRingBuffer<Imu::Ptr> imuBuf) {
    ...
    if (!imuBuf.empty()) {
      ...
      const Eigen::Quaterniond rot_first =
          imuBuf.measMap_.begin()->second->q_w_i;
    ...
    for (std::map<double, Imu::Ptr>::iterator itMeas_ = imuBuf.measMap_.begin();
         itMeas_ != imuBuf.measMap_.end(); ++itMeas_) {
      ...
      acc_cov =
          acc_cov * (Num - 1.0) / Num +
          (acc_cur - acc_mean).cwiseProduct(acc_cur - acc_mean) / (Num - 1.0);
      gyr_cov =
          gyr_cov * (Num - 1.0) / Num +
          (gyr_cur - gyr_mean).cwiseProduct(gyr_cur - gyr_mean) / (Num - 1.0);
      Num++;
    }

    // if (Num > 1 && time_diff_count > 0) {
    //     // Get total time span
    //     double total_time_span = imuBuf.measMap_.rbegin()->second->time -
    //     imuBuf.measMap_.begin()->second->time;
    ...
    Roll_Pitch_Gravity_Matrix =
        calculatePitchRollMatrix(acc_mean.x(), acc_mean.y(), acc_mean.z());
    ...
    std::cout << "imu_laser_extrinsic_gravity: " << imu_laser_gravity_Transform
              << std::endl;
    ...
    std::cout << "pitch offset gravity: " << pitch_offset_gravity * 180 / M_PI
              << std::endl;
    std::cout << "roll offset gravity: " << roll_offset_gravity * 180 / M_PI
              << std::endl;
    std::cout << "Roll Pitch Gravity Matrix: " << Roll_Pitch_Gravity_Matrix
              << std::endl;
  }
