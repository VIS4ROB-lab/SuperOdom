...
bool readPointCloud(const std::string& file_path,
...
bool readLocalizationPose(const std::string& file_path,
                          std::vector<OdometryData>& odometry_results) {
  ...
  while (std::getline(file, line)) {
    ...
    OdometryData odom;
    if (iss >> odom.duration >> odom.x >> odom.y >> odom.z >> odom.roll >>
        odom.pitch >> odom.yaw) {
      std::cout << "Read odometry data: " << odom.x << " " << odom.y << " "
                << odom.z << std::endl;
  ...
  if (!odometry_results.empty()) {
    std::cout
        << "\033[1;32m Loaded the localization_pose.txt successfully \033[0m"
...
bool saveLocalizationPose(double timestamp, const Transformd& T_w_lidar,
  ...
  if (lastSlashPos != std::string::npos) {
    saveOdomPath =
        file_path.substr(0, lastSlashPos + 1);  // Include the trailing slash
  } else {
    saveOdomPath = "./";
  }
  ...
  outFile << std::fixed
          << (odometry_results.size() > 1
                  ? (odom.timestamp - odometry_results[0].timestamp)
                  : 0.0)
          << " " << odom.x << " " << odom.y << " " << odom.z << " " << odom.roll
          << " " << odom.pitch << " " << odom.yaw << std::endl;

  outFile.close();
  return true;
}

void transformAssociateToMap(Transformd& T_w_curr, const Transformd& T_w_pre,
...
void pointAssociateToMap(pcl::PointXYZHSV const* const pi,
                         pcl::PointXYZHSV* const po,
                         const Eigen::Quaterniond& q_w_curr,
                         const Eigen::Vector3d& t_w_curr) {
...
tf2::Quaternion extractRollPitch(Eigen::Quaterniond& imu_rotation) {
  ...
  tf2::Quaternion orientation(imu_rotation.x(), imu_rotation.y(),
                              imu_rotation.z(), imu_rotation.w());
  ...
  RCLCPP_INFO(rclcpp::get_logger("super_odometry"),
              "Using IMU Roll Pitch in ICP: %f %f %f", imu_roll, imu_pitch,
              imu_yaw);
  return quat;
}
...
void transformOusterPoints(point_os::OusterPointXYZIRT const* const pi,
                           point_os::PointcloudXYZITR* const po,
...
void transformHesaiPoints(point_os::HesaiPointXYZIRT const* const pi,
                          point_os::PointcloudXYZITR* const po,
                          Transformd& transform) {
  Eigen::Vector3d point_curr(pi->x, pi->y, pi->z);
  Eigen::Vector3d point_w = transform.rot * point_curr + transform.pos;
...
bool savePly(pcl::PointCloud<PointType>::Ptr pcl_to_save,
  ...
  if (pcl_to_save->size() >= 10 && pcl_to_save->size() % 10 == 0) {
    ...
    if (pcl::io::savePLYFileBinary(all_points_dir, *pcl_to_save) == 0) {
      RCLCPP_INFO(node->get_logger(), "All scans saved to %s with %zu points",
                  all_points_dir.c_str(), pcl_to_save->size());
      return true;
    } else {
      RCLCPP_ERROR(node->get_logger(), "Failed to save scans to %s",
