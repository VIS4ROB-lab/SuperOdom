...
void laserMapping::initInterface() {
  ...
  if (!readGlobalparam(shared_from_this())) {
    RCLCPP_ERROR(
        this->get_logger(),
        "[SuperOdometry::laserMapping] Could not read calibration. Exiting...");
    rclcpp::shutdown();
  }
  ...
  if (!readParameters()) {
    RCLCPP_ERROR(
        this->get_logger(),
        "[SuperOdometry::laserMapping] Could not read parameters. Exiting...");
    rclcpp::shutdown();
  }
  ...
  if (!readCalibration(shared_from_this())) {
    RCLCPP_ERROR(
        this->get_logger(),
        "[AriseSlam::laserMapping] Could not read parameters. Exiting...");
    rclcpp::shutdown();
  }

  RCLCPP_INFO(this->get_logger(), "DEBUG VIEW: %d", config_.debug_view_enabled);
  RCLCPP_INFO(this->get_logger(), "ENABLE OUSTER DATA: %d",
              config_.enable_ouster_data);
  RCLCPP_INFO(
      this->get_logger(),
      "line resolution %f plane resolution %f vision_laser_time_offset %f",
      config_.lineRes, config_.planeRes, vision_laser_time_offset);

  downSizeFilterCorner.setLeafSize(config_.lineRes, config_.lineRes,
                                   config_.lineRes);
  downSizeFilterSurf.setLeafSize(config_.planeRes, config_.planeRes,
                                 config_.planeRes);

  subLaserFeatureInfo =
      this->create_subscription<super_odometry_msgs::msg::LaserFeature>(
  ...
  pubOptimizationStats =
      this->create_publisher<super_odometry_msgs::msg::OptimizationStats>(
  ...
  process_timer_ =
      this->create_wall_timer(std::chrono::milliseconds(static_cast<int>(100.)),
...
void laserMapping::initializationParam() {
  ...
  slam.localMap.setOrigin(
      Eigen::Vector3d(slam.init_x, slam.init_y, slam.init_z));

  if (slam.localization_mode) {
    RCLCPP_INFO(this->get_logger(),
                "\033[1;32m Loading GT Map now.... Please wait for 10 sec "
                "before running rosbag.\033[0m");
    if (utils::readPointCloud(config_.map_dir, laserCloudPrior)) {
      ...
      RCLCPP_INFO(this->get_logger(),
                  "\033[1;32m Loading GT Map Succesfully. Localization mode is "
                  "Ready.\033[0m");
    } else {
      slam.localization_mode = false;
      RCLCPP_INFO(
          this->get_logger(),
          "\033[1;32mCannot read map file, switch to mapping mode.\033[0m");
    }
  } else {
    RCLCPP_INFO(this->get_logger(),
                "\033[1;32mStart SLAM in mapping mode.\033[0m");
  }
}
...
bool laserMapping::readParameters() {
  ...
  this->declare_parameter("laser_mapping_node.publish_only_feature_points",
                          false);
  ...
  this->declare_parameter("laser_mapping_node.velocity_failure_threshold",
                          30.0);
  ...
  this->declare_parameter("laser_mapping_node.localization_mode",
                          false);  // Add default value!
  ...
  // Get parameters
  config_.lineRes =
      this->get_parameter("laser_mapping_node.mapping_line_resolution")
          .as_double();
  config_.planeRes =
      this->get_parameter("laser_mapping_node.mapping_plane_resolution")
          .as_double();
  config_.max_iterations =
      this->get_parameter("laser_mapping_node.max_iterations").as_int();
  config_.debug_view_enabled =
      this->get_parameter("laser_mapping_node.debug_view").as_bool();
  config_.enable_ouster_data =
      this->get_parameter("laser_mapping_node.enable_ouster_data").as_bool();
  config_.publish_only_feature_points =
      this->get_parameter("laser_mapping_node.publish_only_feature_points")
          .as_bool();
  // config_.use_imu_roll_pitch =
  // this->get_parameter("laser_mapping_node.use_imu_roll_pitch").as_bool();
  config_.max_surface_features =
      this->get_parameter("laser_mapping_node.max_surface_features").as_int();
  config_.velocity_failure_threshold =
      this->get_parameter("laser_mapping_node.velocity_failure_threshold")
          .as_double();
  config_.auto_voxel_size =
      this->get_parameter("laser_mapping_node.auto_voxel_size").as_bool();
  config_.forget_far_chunks =
      this->get_parameter("laser_mapping_node.forget_far_chunks").as_bool();
  config_.visual_confidence_factor =
      this->get_parameter("laser_mapping_node.visual_confidence_factor")
          .as_double();
  config_.map_dir = this->get_parameter("map_dir").as_string();
  config_.localization_mode =
      this->get_parameter("laser_mapping_node.localization_mode").as_bool();
  config_.read_pose_file =
      this->get_parameter("laser_mapping_node.read_pose_file").as_bool();
  config_.use_imu_roll_pitch = false;  // Always false for now
  ...
  if (config_.read_pose_file) {
    ...
    config_.init_roll =
        get_parameter("laser_mapping_node.init_roll").as_double();
    config_.init_pitch =
        get_parameter("laser_mapping_node.init_pitch").as_double();
...
void laserMapping::laserFeatureInfoHandler(
    const super_odometry_msgs::msg::LaserFeature::SharedPtr msgIn) {
  mBuf.lock();
  ...
  Eigen::Quaterniond imuprediction_tmp(
...
void laserMapping::initializeFirstFrame() {
  ...
  if (sensorMeas.imuPrediction.w() != 0) {  // Have IMU data
    ...
    tf2::Quaternion initial_orientation =
        utils::extractRollPitch(sensorMeas.imuPrediction);
    q_w_curr =
        Eigen::Quaterniond(initial_orientation.w(), initial_orientation.x(),
    ...
    q_w_curr = Eigen::Quaterniond(1, 0, 0,
                                  0);  // If no IMU data, use identity rotation
  }
  ...
  if (slam.localization_mode) {
    ...
    T_w_lidar.rot =
        Eigen::Quaterniond(localization_pose.w(), localization_pose.x(),
...
void laserMapping::initializeWithIMU() {
  if (sensorMeas.imuPrediction.w() != 0) {  // Have IMU data
    ...
    tf2::Quaternion curr_imu(
        sensorMeas.imuPrediction.w(), sensorMeas.imuPrediction.x(),
    ...
    // Update rotation
    q_w_curr = Eigen::Quaterniond(curr_imu.w(), curr_imu.x(), curr_imu.y(),
                                  curr_imu.z());
    T_w_lidar.rot = q_w_curr;
...
void laserMapping::selectPosePrediction() {
  ...
  switch (prediction_source) {
    ...
    case PredictionSource::IMU_ORIENTATION: {
      Eigen::Quaterniond q_w_predict =
          q_w_curr * q_wodom_pre.inverse() * q_wodom_curr;
...
laserMapping::PredictionSource laserMapping::determinePredictionSource() {
  ...
  if (slam.isDegenerate) {
    ...
    sensorMeas.imu_orientation_status =
        useIMUPrediction(sensorMeas.imuPrediction);
...
void laserMapping::publishTopic() {
  ...
  switch (prediction_source) {
    ...
    case PredictionSource::VIO_ODOM:
      ...
      break;
    case PredictionSource::NEURAL_IMU_ODOM:
      prediction_source_msg.data =
          "Using Neural-Inertial Odometry (Neural-IMU)";
      break;
    case PredictionSource::CONSTANT_VELOCITY:
  ...
  for (int i = 0; i < laserCloudFullResNum; i++) {
    ...
    utils::pointAssociateToMap(&laserCloudFullRes->points[i],
                               &laserCloudFullRes->points[i], q_w_curr,
                               t_w_curr);
  }
  ...
  for (int i = 0; i < laserCloudFullResNum; i++) {
    ...
    laserCloudFullRes_rot->points[i].intensity =
        laserCloudFullRes->points[i].intensity;
  }
  ...
  rclcpp::Time pub_time =
      rclcpp::Clock{RCL_ROS_TIME}
          .now();  // PARV_TODO - find how to syncrynoise this with rosbag time
...
laserMapping::SensorData laserMapping::extractSensorData() {
  ...
  // 4 set status for prediction source (TODO: didn't release code other
  // prediction source yet)
...
void laserMapping::performSLAMOptimization() {
  ...
  if (config_
          .use_imu_roll_pitch) {  // TODO: Livox mid360 not use roll pitch angle
  ...
  slam.Localization(initialization,
                    static_cast<LidarSLAM::PredictionSource>(prediction_source),
                    T_w_lidar, laserCloudCornerStack, laserCloudSurfStack,
                    timeLaserOdometry);
}
...
void laserMapping::process() {
  while (rclcpp::ok()) {
    ...
    try {
      ...
      RCLCPP_ERROR(this->get_logger(), "Error in frame processing: %s",
                   e.what());
    }
  }
}
