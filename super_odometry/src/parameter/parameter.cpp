...
Transformd T_hesai_sensor;

Eigen::Matrix3d hesai_sensor_R;

Eigen::Vector3d hesai_sensor_T;
...
bool readCalibration(rclcpp::Node::SharedPtr node) {
  ...
  RCLCPP_INFO(node->get_logger(), "[super_odometry] calib_file: %s",
              calib_file.c_str());
  ...
  PROVIDE_IMU_LASER_EXTRINSIC =
      node->declare_parameter("provide_imu_laser_extrinsic", true);
  RCLCPP_INFO(node->get_logger(), "PROVIDE_IMU_LASER_EXTRINSIC: %d",
              PROVIDE_IMU_LASER_EXTRINSIC);
  ...
  RCLCPP_INFO(
      node->get_logger(),
      "up realsense extrinsic to velodyne (RPYXYZ): %f, %f, %f, %f, %f, %f",
      up_realsense_roll, up_realsense_pitch, up_realsense_yaw, up_realsense_x,
      up_realsense_y, up_realsense_z);

  RCLCPP_INFO(
      node->get_logger(),
      "down realsense extrinsic to velodyne (RPYXYZ): %f, %f, %f, %f, %f, %f",
      down_realsense_roll, down_realsense_pitch, down_realsense_yaw,
      down_realsense_x, down_realsense_y, down_realsense_z);
  ...
  if (PROVIDE_IMU_LASER_EXTRINSIC) {
    ...
    tf2::Quaternion orientation_pre(T_i_l.rot.x(), T_i_l.rot.y(), T_i_l.rot.z(),
                                    T_i_l.rot.w());
    ...
    imu_laser_rot = Eigen::Quaterniond(IMU_LASER.w(), IMU_LASER.x(),
                                       IMU_LASER.y(), IMU_LASER.z());
    ...
    tf2::Quaternion orientation_curr(IMU_LASER.x(), IMU_LASER.y(),
                                     IMU_LASER.z(), IMU_LASER.w());
    tf2::Matrix3x3(orientation_curr)
        .getRPY(updated_roll, updated_pitch, updated_yaw);

    RCLCPP_INFO(node->get_logger(),
                GREEN BOLD
                "\n updated roll: %f updated pitch: %f updated yaw: %f" RESET,
                updated_roll * 180 / M_PI, updated_pitch * 180 / M_PI,
                updated_yaw * 180 / M_PI);
  ...
  ouster_sensor_R << -1, 0, 0, 0, -1, 0, 0, 0, 1;

  ouster_sensor_T << 0, 0, 0.036180;

  T_ouster_sensor = Transformd(ouster_sensor_R, ouster_sensor_T);

  hesai_sensor_R << 1, 0, 0, 0, 1, 0, 0, 0, 1;

  hesai_sensor_T << 0, 0, 0;

  T_hesai_sensor = Transformd(hesai_sensor_R, hesai_sensor_T);
...
bool readGlobalparam(rclcpp::Node::SharedPtr node) {
  ...
  node->declare_parameter<std::string>("depthup_topic",
                                       "/rs_up/depth/cloud_filtered");
  node->declare_parameter<std::string>("depthdown_topic",
                                       "/rs_down/depth/cloud_filtered");
  ...
  RCLCPP_INFO(node->get_logger(), "DepthDown_TOPIC %s",
              DepthDown_TOPIC.c_str());
  RCLCPP_INFO(node->get_logger(), "WORLD_FRAME %s", WORLD_FRAME.c_str());
  RCLCPP_INFO(node->get_logger(), "WORLD_FRAME_ROT %s",
              WORLD_FRAME_ROT.c_str());
  RCLCPP_INFO(node->get_logger(), "SENSOR_FRAME %s", SENSOR_FRAME.c_str());
  RCLCPP_INFO(node->get_logger(), "SENSOR_FRAME_ROT %s",
              SENSOR_FRAME_ROT.c_str());
