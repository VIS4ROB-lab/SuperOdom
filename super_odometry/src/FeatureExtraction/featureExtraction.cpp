...
void featureExtraction::initInterface() {
  ...
  if (!readGlobalparam(shared_from_this())) {
    RCLCPP_ERROR(this->get_logger(),
                 "[super_odometry::featureExtraction] Could not read "
                 "calibration. Exiting...");
    rclcpp::shutdown();
  }
  if (!readParameters()) {
  ...
  if (config_.N_SCANS != 16 && config_.N_SCANS != 32 && config_.N_SCANS != 64 &&
      config_.N_SCANS != 4 && config_.N_SCANS != 128) {
    RCLCPP_ERROR(this->get_logger(),
                 "only support velodyne, livox, ouster with 16, 32, 64 or 128 "
                 "scan line! and livox mid 360");
    rclcpp::shutdown();
  }
  ...
  subImu = this->create_subscription<sensor_msgs::msg::Imu>(
      IMU_TOPIC, imu_qos,
      std::bind(&featureExtraction::imu_Handler, this, std::placeholders::_1),
      sub_options);
...
bool featureExtraction::readParameters() {
  ...
  this->declare_parameter<bool>("feature_extraction_node.use_dynamic_mask",
                                false);
  this->declare_parameter<bool>("feature_extraction_node.use_imu_roll_pitch",
                                false);
  ...
  this->declare_parameter<double>("feature_extraction_node.imu_acc_x_limit",
                                  1.0);
  this->declare_parameter<double>("feature_extraction_node.imu_acc_y_limit",
                                  1.0);
  this->declare_parameter<double>("feature_extraction_node.imu_acc_z_limit",
                                  1.0);
  this->declare_parameter<std::string>("feature_extraction_node.sensor",
                                       "livox");

  config_.N_SCANS =
      this->get_parameter("feature_extraction_node.scan_line").as_int();
  config_.skipFrame =
      this->get_parameter("feature_extraction_node.mapping_skip_frame")
          .as_int();
  config_.box_size.blindFront =
      this->get_parameter("feature_extraction_node.blindFront").as_double();
  config_.box_size.blindBack =
      this->get_parameter("feature_extraction_node.blindBack").as_double();
  config_.box_size.blindLeft =
      this->get_parameter("feature_extraction_node.blindLeft").as_double();
  config_.box_size.blindRight =
      this->get_parameter("feature_extraction_node.blindRight").as_double();
  // config_.use_imu_roll_pitch =
  // this->get_parameter("feature_extraction_node.use_imu_roll_pitch").as_bool();
  config_.min_range =
      this->get_parameter("feature_extraction_node.min_range").as_double();
  config_.max_range =
      this->get_parameter("feature_extraction_node.max_range").as_double();
  config_.filter_point_size =
      this->get_parameter("feature_extraction_node.filter_point_size").as_int();
  config_.provide_point_time =
      this->get_parameter("feature_extraction_node.provide_point_time")
          .as_int();
  config_.use_dynamic_mask =
      this->get_parameter("feature_extraction_node.use_dynamic_mask").as_bool();
  config_.debug_view_enabled =
      this->get_parameter("feature_extraction_node.debug_view").as_bool();
  config_.imu_acc_x_limit =
      this->get_parameter("feature_extraction_node.imu_acc_x_limit")
          .as_double();
  config_.imu_acc_y_limit =
      this->get_parameter("feature_extraction_node.imu_acc_y_limit")
          .as_double();
  config_.imu_acc_z_limit =
      this->get_parameter("feature_extraction_node.imu_acc_z_limit")
          .as_double();
  config_.use_imu_roll_pitch = false;
...
bool featureExtraction::synchronize_measurements(
    MapRingBuffer<Meas>& measureBuf,
    MapRingBuffer<pcl::PointCloud<point_os::PointcloudXYZITR>::Ptr>& lidarBuf) {
  if (lidarBuf.getSize() == 0 or measureBuf.getSize() == 0) return false;
  ...
  if (meas_end_time <=
      lidar_end_time)  // make sure imu message arrives after lidar message
  {
    RCLCPP_WARN_STREAM(this->get_logger(),
                       "meas_end_time < lidar_end_time ||"
                       " message order is not perfect! please restart velodyne "
                       "and imu driver!");
    RCLCPP_WARN(this->get_logger(), "meas_end_time %f <  %f lidar_end_time",
                meas_end_time, lidar_end_time);
    RCLCPP_WARN(this->get_logger(),
                "All the lidar data is more recent than all the imu data. Will "
                "throw away lidar frame");
  ...
  if (meas_start_time >= lidar_start_time) {
    RCLCPP_WARN(this->get_logger(),
                "throw laser scan, only should happen at the beginning");
    lidarBuf.clean(lidar_start_time);
    RCLCPP_WARN(this->get_logger(),
                "removed the lidarBuf size % d, measureBuf size % d ",
                lidarBuf.getSize(), measureBuf.getSize());
    RCLCPP_WARN(this->get_logger(),
                "meas_start_time: %f > lidar_start_time: %f ", meas_start_time,
                lidar_start_time);
    return false;
  } else {
    return true;
...
void featureExtraction::removePointDistortion(
    double lidar_start_time, double lidar_end_time,
  ...
  auto extractPose = [](const BufferType& data) -> Transformd {
    ...
    if constexpr (std::is_same_v<BufferType, Imu::Ptr>) {
      ...
      pose.pos = Eigen::Vector3d(data->pose.pose.position.x,
  ...
  auto getInterpolatedPoseAtTime =
      [&buffer, &extractPose](double timestamp) -> Transformd {
  ...
  Transformd T_w_original_sensor =
      is_imu_data ? T_w_original * T_i_l : T_w_original;
  ...
  for (auto& point : lidar_msg->points) {
    ...
    Transformd T_final =
        is_imu_data ? T_l_i * T_original_current * T_i_l : T_original_current;
...
Transformd featureExtraction::getInterpolatedPose(
    double timestamp, MapRingBuffer<BufferType>& buffer,
  ...
  auto before_ptr = std::prev(after_ptr);
  double ratio =
      (timestamp - before_ptr->first) / (after_ptr->first - before_ptr->first);
...
Eigen::Vector3d featureExtraction::transformPoint(
    const point_os::PointcloudXYZITR& point, const Transformd& T_w_original,
...
void featureExtraction::updatePointPosition(point_os::PointcloudXYZITR& point,
...
sensor_msgs::msg::PointCloud2 featureExtraction::publishCloud(
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr thisPub,
    typename pcl::PointCloud<Point>::Ptr thisCloud, rclcpp::Time thisStamp,
...
void featureExtraction::publishTopic(
    double lidar_start_time,
  ...
  laserFeature.cloud_nodistortion = publishCloud<point_os::PointcloudXYZITR>(
      pubLaserCloud, laser_no_distortion_points, FeatureHeader.stamp,
      SENSOR_FRAME);
  laserFeature.cloud_corner = publishCloud<PointType>(
      pubEdgePoints, edgePoints, FeatureHeader.stamp, SENSOR_FRAME);
  laserFeature.cloud_surface = publishCloud<PointType>(
      pubPlannerPoints, plannerPoints, FeatureHeader.stamp, SENSOR_FRAME);
  laserFeature.cloud_realsense = publishCloud<PointType>(
      pubBobPoints, depthPoints, FeatureHeader.stamp, SENSOR_FRAME);
...
void featureExtraction::extractFeatures(
  ...
  uniformFeatureExtraction(lidar_msg, plannerPoints, config_.filter_point_size,
                           config_.min_range);

  publishTopic(lidar_start_time, lidar_msg, edgePoints, plannerPoints,
               bobPoints, quaternion);
}
...
void featureExtraction::undistortionAndFeatureExtraction() {
  ...
  LASER_CAMERA_SYNC_SUCCESS =
      synchronize_measurements<nav_msgs::msg::Odometry::SharedPtr>(
          visualOdomBuf, lidarBuf);
  ...
  if ((LASER_IMU_SYNC_SCCUESS == true or LASER_CAMERA_SYNC_SUCCESS == true) and
      lidarBuf.getSize() > 0) {
    double lidar_start_time;
    lidarBuf.getFirstTime(lidar_start_time);
    pcl::PointCloud<point_os::PointcloudXYZITR>::Ptr lidar_msg;
    ...
    if (LASER_IMU_SYNC_SCCUESS == true and LASER_CAMERA_SYNC_SUCCESS == true) {
      RCLCPP_INFO(
          this->get_logger(),
          "\033[1;32m----> Both IMU ,VIO laserscan are synchronized!.\033[0m");
      removePointDistortion<nav_msgs::msg::Odometry::SharedPtr>(
          lidar_start_time, lidar_end_time, visualOdomBuf, lidar_msg);
    }
    ...
    if (LASER_IMU_SYNC_SCCUESS == true and LASER_CAMERA_SYNC_SUCCESS == false) {
      // RCLCPP_INFO(this->get_logger(), "\033[1;32m----> IMU and laserscan is
      // synchronized!.\033[0m");
      removePointDistortion<Imu::Ptr>(lidar_start_time, lidar_end_time, imuBuf,
                                      lidar_msg);
    }
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "\033[1;32m----> no IMU data, running LiDAR Odometry only.\033[0m");
...
void featureExtraction::uniformFeatureExtraction(
    const pcl::PointCloud<point_os::PointcloudXYZITR>::Ptr& pc_in,
    pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_out_surf, int skip_num,
    float block_range) {
...
ImuMeasurement featureExtraction::parseImuMessage(
    const sensor_msgs::msg::Imu::SharedPtr& msg) {
  ImuMeasurement measurement;
  measurement.timestamp =
      msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9;
  measurement.accel << msg->linear_acceleration.x, msg->linear_acceleration.y,
      msg->linear_acceleration.z;
  measurement.gyr << msg->angular_velocity.x, msg->angular_velocity.y,
...
void featureExtraction::updateImuOrientation(Imu::Ptr& imudata) {
  if (!imuBuf.empty()) {
    ...
    tf2::Quaternion orientation_curr(imudata->q_w_i.x(), imudata->q_w_i.y(),
                                     imudata->q_w_i.z(), imudata->q_w_i.w());
    ...
    imudata->q_w_i =
        Eigen::Quaterniond(first_orientation.w(), first_orientation.x(),
...
void featureExtraction::imu_Handler(
...
void featureExtraction::visual_odom_Handler(
    const nav_msgs::msg::Odometry::SharedPtr visualOdometry) {
  m_buf.lock();
  visualOdomBuf.addMeas(visualOdometry,
                        visualOdometry->header.stamp.sec +
                            visualOdometry->header.stamp.nanosec * 1e-9);
  m_buf.unlock();
}

void featureExtraction::assignTimeforPointCloud(
    pcl::PointCloud<PointType>::Ptr laserCloudIn_ptr_) {
  ...
  for (size_t i = 0; i < cloud_size; i++) {
    ...
    point.ring = scanID;
    float rel_time = (columnTime * int(i / config_.N_SCANS) +
                      laserTime * (i % config_.N_SCANS)) /
                     scanPeriod;
...
void featureExtraction::laserCloudHandler(
  ...
  manageLidarBuffer(pointCloud, laserCloudMsg->header.stamp.sec +
...
void featureExtraction::livoxHandler(
  ...
  if (config_.provide_point_time) {
    for (uint i = 0; i < msg->point_num; i++) {
      if ((msg->points[i].line < config_.N_SCANS) &&
        ...
        Eigen::Vector3d point(msg->points[i].x, msg->points[i].y,
                              msg->points[i].z);
        ...
        pointCloud->points[i].time =
            msg->points[i].offset_time / float(1000000000);
        pointCloud->points[i].ring = msg->points[i].line;
      }
    }
  } else {
    RCLCPP_ERROR(this->get_logger(),
                 "Please check yaml or livox driver to provide the timestamp "
                 "for each point");
    rclcpp::shutdown();
  }

  manageLidarBuffer(pointCloud,
                    msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9);
